...
class LatentDiffusion(nn.Module):
    ...
    def forward(self, x): #, c):#, *args, **kwargs):
        ...
        t = torch.tensor([self.num_timesteps], device=self.device).repeat(x.shape[0]).long()
        # print(t.shape)
        # print(concat_x_t.shape)
        return self.p_losses(x, t)#, *args, **kwargs)

    def from_noisy(self, x_start, tmax=None):
        ...
        t = torch.tensor([tmax]).repeat(x_shape)
...
# prep images for display
gen_latent_1 = stab_diff_test.gen_noisy(gt_latent, tmax=1)
gen_latent_5 = stab_diff_test.gen_noisy(gt_latent, tmax=5)
gen_latent_10 = stab_diff_test.gen_noisy(gt_latent, tmax=10)
gen_latent_15 = stab_diff_test.gen_noisy(gt_latent, tmax=15)
gen_latent_20 = stab_diff_test.gen_noisy(gt_latent, tmax=20)

output = model.decode(gt_latent)
gen_output_1 = model.decode(gen_latent_1)
gen_output_5 = model.decode(gen_latent_5)
gen_output_10 = model.decode(gen_latent_10)
gen_output_15 = model.decode(gen_latent_15)
gen_output_20 = model.decode(gen_latent_20)

ground_truth_images = torch.stack([resize_transform(image) for image in images]) 
ae_gen_image = torch.stack([resize_transform(image) for image in output])
generated_images_1 = torch.stack([resize_transform(image) for image in gen_output_1])
generated_images_5 = torch.stack([resize_transform(image) for image in gen_output_5])
generated_images_10 = torch.stack([resize_transform(image) for image in gen_output_10])
generated_images_15 = torch.stack([resize_transform(image) for image in gen_output_15])
generated_images_20 = torch.stack([resize_transform(image) for image in gen_output_20])
# output is resized into a batch of iages
print(generated_images.shape)
# output = output.view(batch_size, 1, 28, 28)
# use detach when it's an output that requires_grad
...
output3 = generated_images_1.cpu().detach().numpy()
output4 = generated_images_5.cpu().detach().numpy()
output5 = generated_images_10.cpu().detach().numpy()
output6 = generated_images_15.cpu().detach().numpy()
output7 = generated_images_20.cpu().detach().numpy()

# plot the first ten input images and then reconstructed images
fig, axes = plt.subplots(nrows=8, ncols=10, sharex=True, sharey=True, figsize=(10,6))

# input images on top row, reconstructions on bottom
for gt, row in zip([ground_truth_images, output1, output2, output3, output4, output5, output6, output7], axes):
    for img, ax in zip(gt, row):
        ax.imshow(np.squeeze(img), cmap='gray')
        ax.get_xaxis().set_visible(False)
        ax.get_yaxis().set_visible(False)
...
print("without denoise, t_max 1 to 20")
plt.savefig(f"{now}_{num_qubits}_{num_layer}_without_denoise.png",format="png")

# %%
# sampler = DDIMSampler(stab_diff_test)
...
# with torch.no_grad():
#     with stab_diff_test.ema_scope():
#         print(f"rendering {n_samples_per_class} in {ddim_steps} steps and using s={scale:.2f}.")

#         samples_ddim, _ = sampler.sample(S=ddim_steps,
#                                             batch_size=n_samples_per_class,
#                                             shape=[16],  # isit...shape of...latent...? or shape of... image...
#                                             verbose=True,
#                                             eta=ddim_eta)
#         print("sample:",samples_ddim)
#         # x_samples_ddim = model.decode(nn.Sigmoid()(samples_ddim))
#         x_samples_ddim = model.decode(samples_ddim)
...
#         # samples_ddim, _ = sampler.sample(S=ddim_steps,
#         #                                     batch_size=n_samples_per_class,
#         #                                     shape=[32],  # isit...shape of...latent...? or shape of... image...
#         #                                     verbose=True,
#         #                                     eta=ddim_eta)
#         samples_ddim = torch.randn((6,16))
#         # print(samples_ddim)
#         x_samples_ddim = model.decode(nn.Sigmoid()(samples_ddim))
#         # x_samples_ddim = model.decode(samples_ddim)
#         # x_samples_ddim = torch.clamp((x_samples_ddim+1.0)/2.0, 
...
# fig, ax = qml.draw_mpl(stab_diff_test.circuit, all_wires=True)(torch.randn((1,pow(2,num_qubits)+1)))
