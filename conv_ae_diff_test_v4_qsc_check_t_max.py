...
# prep images for display
gen_latent = stab_diff_test.from_noisy(gt_latent)

gen_latent_min = stab_diff_test.gen_noisy(gt_latent, tmax=1)
gen_latent_max = stab_diff_test.gen_noisy(gt_latent, tmax=10)

output = model.decode(gt_latent)
gen_output = model.decode(gen_latent)
gen_output_min = model.decode(gen_latent_min)
gen_output_max = model.decode(gen_latent_max)
...
generated_images_min = torch.stack([resize_transform(image) for image in gen_output_min])
generated_images_max = torch.stack([resize_transform(image) for image in gen_output_max])
# output is resized into a batch of iages
print(generated_images.shape)
# output = output.view(batch_size, 1, 28, 28)
# use detach when it's an output that requires_grad
...
output3 = generated_images_min.cpu().detach().numpy()
output4 = generated_images_max.cpu().detach().numpy()
...
print(f"t_max as {T_max}")
plt.savefig(f"{now}_{num_qubits}_{num_layer}_l2_{T_max}.png",format="png")
